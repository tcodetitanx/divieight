...
class Shortcode_Seller_Login {
	...
	public function add_shortcode(): void {
		if ( ( ! wp_doing_ajax() ) && ( ! is_admin() ) ) {
			add_shortcode( 'hre_seller_elite_login', array( $this, 'load_view' ) );
		}
	}
	...
	public function load_view(): string {
		 ...
		 * @since 1.0.0
		 */
		do_action( 'hre_enqueue_shortcode_seller_login' );
	...
	public function register_scripts(): void {
		$css = mp_get_style( '/public/sc-seller-login' );
		$js  = mp_get_script( '/public/sc-seller-login' );

		wp_register_style( 'hre-shortcode-seller-login', $css, array(), Initializer::$script_version );
		wp_register_script(
			'hre-shortcode-seller-login',
			$js,
		...
		// enqueue the scripts.
		add_action(
			'hre_enqueue_shortcode_seller_login',
			function () {
				wp_enqueue_style( 'hre-shortcode-seller-login' );
				wp_enqueue_script( 'hre-shortcode-seller-login' );
