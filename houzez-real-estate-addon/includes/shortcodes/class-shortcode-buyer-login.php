...
class Shortcode_Buyer_Login {
	 ...
	 * @return $this
	 */
	public function init(): Shortcode_Buyer_Login {
	...
	public function add_shortcode(): void {
		if ( ( ! wp_doing_ajax() ) && ( ! is_admin() ) ) {
			add_shortcode( 'hre_buyer_elite_login', array( $this, 'load_view' ) );
		}
	}
	...
	public function load_view(): string {
		 ...
		 * @since 1.0.0
		 */
		do_action( 'hre_enqueue_shortcode_buyer_login' );

		return sprintf(
			'<div class="hre-shortcode-login" >%1$s</div>',
			esc_attr__(
				'Loading...',
				'hre-addon'
			)
	...
	public function register_scripts(): void {
		$css = mp_get_style( '/public/sc-buyer-login' );
		$js  = mp_get_script( '/public/sc-buyer-login' );

		wp_register_style( 'hre-shortcode-buyer-login', $css, array(), Initializer::$script_version );
		wp_register_script(
			'hre-shortcode-buyer-login',
			$js,
		...
		// enqueue the scripts.
		add_action(
			'hre_enqueue_shortcode_buyer_login',
			function () {
				/**
				 ...
				 * @since 1.0.0
				 */
				wp_enqueue_style( 'hre-shortcode-buyer-login' );
				wp_enqueue_script( 'hre-shortcode-buyer-login' );
