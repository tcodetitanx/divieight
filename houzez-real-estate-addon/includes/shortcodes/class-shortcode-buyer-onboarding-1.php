...
/**
 * Class Shortcode_Buyer_Onboarding_1
 * Shortcode for buyer onboarding process 1.
 */
class Shortcode_Buyer_Onboarding_1 {
	...
	public function add_shortcode(): void {
		if ( ( ! wp_doing_ajax() ) && ( ! is_admin() ) ) {
			add_shortcode( 'buyer_onboarding_1', array( $this, 'load_view' ) );
		}
	}
	...
	public function load_view(): string {
		 ...
		 * @since 1.0.0
		 */
		do_action( 'hre_enqueue_shortcode_buyer_onboarding_1' );

		return sprintf(
			'<div class="hre-shortcode-buyer-onboarding" >%1$s</div>',
			esc_attr__(
				'Loading...',
				'hre-addon'
			)
	...
	public function register_scripts(): void {
		$css = mp_get_style( '/public/sc-buyer-onboarding-1' );
		$js  = mp_get_script( '/public/sc-buyer-onboarding-1' );

		wp_register_style( 'hre-shortcode-buyer-onboarding-1', $css, array(), Initializer::$script_version );
		wp_register_script(
			'hre-shortcode-buyer-onboarding-1',
			$js,
		...
		// enqueue the scripts.
		add_action(
			'hre_enqueue_shortcode_buyer_onboarding_1',
			function () {
				/**
				 ...
				 * @since 1.0.0
				 */
				wp_enqueue_style( 'hre-shortcode-buyer-onboarding-1' );
				wp_enqueue_script( 'hre-shortcode-buyer-onboarding-1' );
